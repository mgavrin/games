        ...
        for ship in self.shiplist:
            ...
            for square in ship:
                ...
                if squareVal!=2:
                    ...
                    if self.count_status==False:
                        self.count_status=True
            if sunk==True:
                self.turn_counter+=1
                self.time_to_sink.append(self.turn_counter)
                self.turn_counter=0
                self.count_status=False
                for square in ship:
...
class Game:
    def __init__(self):
        ...
        self.fps=70
        self.shot_time=self.fps/50 #1=1 shot per second, 25=25 shots/sec
        self.dim=10
        ...
        while self.running==True:
            ...
            if self.checkEndGame():
                self.Update()
            if self.playing==True and self.drawcount%(self.shot_time)==0:
                self.takeShot(self.Board4,self.Board1,"checker")
                self.takeShot(self.Board3,self.Board2,"random")
                if self.Board3.count_status==True:
                    self.Board3.turn_counter+=1
                if self.Board4.count_status==True:
                    self.Board4.turn_counter+=1
    ...
    def outputGameMessage(self,winner):
        total_time=round(float(self.playcount)/self.fps)
        av_time=round(total_time/self.games,2)
        perc_cmplt=round(float(self.games)/(self.max_games+1)*100,2)
        rem_time=int(av_time*(self.max_games-self.games+1))
        rem_min=rem_time/60
        rem_sec=rem_time%60
        if winner=="draw":
            output= "Game " + str(self.games) + " ended in a draw. "
        elif winner=="P1":
            output= "Game " + str(self.games) + " ended in P1 win. "
        elif winner=="P2":
            output= "Game " + str(self.games) + " ended in P2 win. "
        output+= str(total_time) + " seconds elapsed, "
        output+=str(av_time)+ " games/sec \n"
        output+="Sim run is " + str(perc_cmplt)
        output+="% complete, estimate " + str(rem_min) + "m " + str(rem_sec)
        output+=" seconds left in run."
        return(output)

    def checkEndGame(self):
        endgame=True
        if self.playing==True and self.Board3.empty() and self.Board4.empty():
            print self.outputGameMessage("draw")
            ...
            print self.outputGameMessage("P2")
            if self.games<=self.max_games:
                self.P2_wins+=1
                self.games+=1
                for board in self.boards:
            ...
            print self.outputGameMessage("P1")
            if self.games<=self.max_games:
                self.P1_wins+=1
                self.games+=1
                for board in self.boards:
            ...
            endgame=False
        if self.playing==False and self.games==self.max_games+1:
            #Do post-game stats stuff
            print "Simulation completed, " + str(self.games) + " played."
            output="Player 1 won " + str(self.P1_wins) + " games ("
            output+=str(100*float(self.P1_wins)/self.games) + "%)"
            print output
            output="Player 2 won " + str(self.P2_wins) + " games ("
            output+=str(100*float(self.P2_wins)/self.games) + "%)"
            print output
            output="The game was a draw in " + str(self.draws) + " games ("
            output+=str(100*float(self.draws)/self.games) + "%)"
            print output
            self.running=False
        return(endgame)
    ...
    def Update(self):
        ...
        if self.playing==False:
            try:
                ...
                print "game ended, this is just avoiding a crash"
            else:
                pass

    def pickShot(self,guess,real,mode):
        ...
        if self.playing==True:
            ...
            if len(targets)==0:
                if mode=="checker":
                    ...
                    shot=potlist[0]
        if shot==False:
            print "Arg, this fucking bug!!! #@$% &*@#!!"
        return(shot)
    
    def takeShot(self,real,guess,mode):
        try:
            square=self.pickShot(guess,real,mode)
            (s_row,s_col)=square
        except:
            self.running=False
        else:
